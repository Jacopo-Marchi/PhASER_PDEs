...
fig2, axs_fig2 = plt.subplots(nrows=2, ncols=3, figsize=(12,8),layout='constrained')

    
labeled_axes = []
labeled_axes.append(axs_fig2[0,0])
labeled_axes.append(axs_fig2[0,1])
labeled_axes.append(axs_fig2[0,2])
labeled_axes.append(axs_fig2[1,0])
labeled_axes.append(axs_fig2[1,1])
labeled_axes.append(axs_fig2[1,2])
...
dirstrings=['{inp}/t7_F8real_mf_1_track_nutr_rescpars_highchi_50e6','{inp}/t7_F8real_mf_1_track_nutr_rescpars_highchi_50e6','{inp}/t7_F8real_mf_1_track_nutr_rescpars_highchi_50e6','{inp}/t7_F8real_mf_1_track_nutr_rescpars_highchi_50e6_eta10','{inp}/t7_F8real_mf_1_track_nutr_rescpars_highchi_50e6_eta10','{inp}/t7_F8real_mf_1_track_nutr_rescpars_highchi_50e6_eta10']
titstrings=[r'$\eta=5$',r'$\eta=5$',r'$\eta=5$',r'$\eta=10$',r'$\eta=10$',r'$\eta=10$']
t_ints=[26,40, 56,26,40,48]
...
for idir, dirstr in enumerate(dirstrings):
    ...
    for it, t in enumerate(times):
        ...
        if int(t) == t_int:
            ...
            # ~ heatmap = ax.pcolormesh(xx, yy, intensities, cmap='gray',vmin=maxbacts_convert, vmax=no_bactcontrol) # faster than pcolor
            heatmap = axs_fig2[row,col].pcolormesh(xx, yy, intensities, cmap='gray',vmin=0, vmax=255, shading='auto', linewidth=0,rasterized=True) # faster than pcolor
            # ~ ax.set_xlabel(r'x (mm)')
            # ~ ax.set_ylabel(r'y (mm)')
            
            axs_fig2[row,col].plot((21000  -L/2.)/1000, (21000  -L/2.)/1000, marker='x', markersize=6, color='k', linestyle='')
            axs_fig2[row,col].plot((6000  -L/2.)/1000, (6000  -L/2.)/1000, marker='o',  markerfacecolor='none', markersize=6,   color='k', linestyle='')
            ...
            heatmap = axs_fig2[row,col].pcolormesh(xx, yy,   wavesum ,norm=colors.LogNorm(vmin=100, vmax=200), cmap='gray_r', alpha=0.5) # faster than pcolor, , alpha=0.5
            # ~ heatmap = axs_fig2[row,col].pcolormesh(xx, yy,   wavesum , cmap='gray', alpha=0.5) # faster than pcolor, , alpha=0.5
            
            axs_fig2[row,col].set_aspect('equal')
    
            scalebar = AnchoredSizeBar(axs_fig2[row,col].transData,
                                   25, '25 mm', 'lower right', 
                                   pad=0.2,
                                   color='black',
                                   frameon=False,
                                   size_vertical=0.3, fontproperties=fontprops)
            axs_fig2[row,col].add_artist(scalebar)
          
            trans = transforms.blended_transform_factory(axs_fig2[row,col].transData, axs_fig2[row,col].get_xticklabels()[0].get_transform())
            
            axs_fig2[row,col].annotate(titstrings[idir], xy=(-90, 85), xycoords='data',xytext=(0, 5), textcoords='offset pixels', color='k', fontsize=16)

    
            axs_fig2[row,col].annotate(str(int(t))+" hr", xy=(65, 85), xycoords='data',xytext=(0, 5), textcoords='offset pixels', color='k', fontsize=16)    #
        #
            axs_fig2[row,col].tick_params(left = False, right = False , labelleft = False ,
                        labelbottom = False, bottom = False)
...
out_file='{out}/SIfig_track_nutr.png'.format(out=dir_fig)
