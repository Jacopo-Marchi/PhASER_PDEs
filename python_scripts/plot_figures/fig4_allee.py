...
dirstrings=['{inp}/F8_t7_Vcinf_P05e4','{inp}/F8_t4_Vc1e7_mf0d87_P05e5','{inp}/F8_t4_Vc1e7_mf0d87_P07e5','{inp}/F8_t4_Vc1e7_mf0d87_P01e6','{inp}/F8_t4_Vc1e7_mf0d87_P05e6','{inp}/F8_t7_Vcinf_P05e5','{inp}/F8_t7_Vcinf_P07e5','{inp}/F8_t7_Vcinf_P01e6','{inp}/F8_t7_Vcinf_P05e6']
t_ints=[40,26,36,42,34,42,42,42,42]

figallee, axs_allee = plt.subplots(nrows=2, ncols=(len(dirstrings)+1)//2, figsize=(20,8),  layout='constrained')# , layout='constrained',
    ...
    for file_in_frames in glob.glob("{inp}/experiment_state_time_*".format(inp=dir_in_frames)):
        ...
        if int(t) == t_int :
            ...
            if '5e4' in dirstr:
                ...
                row_plot=int('t7' in dirstr)
                col_plot=idir - int('t7' in dirstr)*(len(dirstrings)-1)//2
    
            v= plot_timelapse_intensity(xx,yy,intensities,axs_allee[row_plot,col_plot],col_plot- (len(dirstrings)-1)//2) #,rescdim=10/6
            
            
            axs_allee[row_plot, col_plot].plot((21000 -L/2.)/1000, (21000 -L/2.)/1000, marker='x', markersize=12, color='k', linestyle='')
            axs_allee[row_plot, col_plot].plot((6000  -L/2.)/1000, (6000  -L/2.)/1000, marker='o',  markerfacecolor='none', markersize=12,   color='k', linestyle='')
            
            if row_plot:
                axs_allee[row_plot, col_plot].set_title("Estimated PFUs: {:.1f}".format(PFU_est), y=-0.1, fontsize=18)
...
out_file='{out}/fig3_allee.png'.format(out=dir_fig)
#    print out_file
