...
dir_fig='../../sims_for_SI_T4_Vcinf' # directory with input files
...
fig2, axs_fig2 = plt.subplots(nrows=2, ncols=2, figsize=(8,8),layout='constrained')

    
labeled_axes = []
labeled_axes.append(axs_fig2[0,0])
# ~ labeled_axes.append(axs_fig2[1,0])
labeled_axes.append(axs_fig2[0,1])
labeled_axes.append(axs_fig2[1,0])
labeled_axes.append(axs_fig2[1,1])




dirstrings=['{inp}/T4_fin_Vcinf_F8','{inp}/merge_T4_Vcinf_60deg_8cmcoll','{inp}/merge_T4_Vcinf_120deg_8cmcoll']
t_ints=[52,42,40]
...
for idir, dirstr in enumerate(dirstrings):
    ...
    # ~ isfindiff=  len(glob.glob("{inp}/experiment_state_time_*.dat".format(inp=dir_in_frames)))>0
    # ~ print isfindiff
    # ~ if isfindiff:
        # ~ pattern="{inp}/experiment_state_time_*.dat".format(inp=dir_in_frames)
    # ~ else:
        # ~ pattern=
    savedat=True
    for file_in_frames in glob.glob("{inp}/experiment_state_time_*".format(inp=dir_in_frames)):
        ...
        if int(t) == t_int:        
            ...
            row= int(idir/2)
            col= np.mod(idir,2)
            v= plot_timelapse_intensity(xx,yy,intensities,axs_fig2[row,col],0) #,rescdim=10/6
            
            
            if idir==0:
                axs_fig2[row,col].plot((21000  -L/2.)/1000, (21000  -L/2.)/1000, marker='x', markersize=3, color='k', linestyle='')
                axs_fig2[row,col].plot((6000  -L/2.)/1000, (6000  -L/2.)/1000, marker='o',  markerfacecolor='none', markersize=3,   color='k', linestyle='')
                ...
                xx_rot_pivot, yy_rot_pivot = rotate((0,0),105000  -L/2.,0, np.deg2rad(-45))
            
                # ~ xx_rot, yy_rot = rotate((76000  -L/2.,0),xx_rot_tmp, np.abs(yy_rot_tmp), np.deg2rad(22.5))
                xx_rot_vir, yy_rot_vir = rotate((xx_rot_pivot,yy_rot_pivot),xx_rot_tmp_vir, yy_rot_tmp_vir, np.deg2rad(deg))
                xx_rot_bacts, yy_rot_bacts = rotate((xx_rot_pivot,yy_rot_pivot),xx_rot_tmp_bacts, yy_rot_tmp_bacts, np.deg2rad(deg))
                
                
                      
                axs_fig2[row,col].plot(xx_rot_vir/1000, yy_rot_vir/1000, marker='x', markersize=3, color='k', linestyle='')
                axs_fig2[row,col].plot(xx_rot_bacts/1000, yy_rot_bacts/1000, marker='o',  markerfacecolor='none', markersize=3,   color='k', linestyle='')
...
dir_io='{inp}/merge_t4_Vcinf_F8_vsF10'.format(inp=dir_fig)  # directory with input files
...
for it, t in enumerate(times):
    ...
    if int(t) in t_ref:
        ...
        print ("PLOTTING LAST")
        # ~ heatmap = ax.pcolormesh(xx, yy, intensities, cmap='gray',vmin=maxbacts_convert, vmax=no_bactcontrol) # faster than pcolor
        heatmap = axs_fig2[1,1].pcolormesh(-xx, yy, intensities, cmap='gray',vmin=0, vmax=255, shading='auto', linewidth=0,rasterized=True) # faster than pcolor
        # ~ ax.set_xlabel(r'x (mm)')
        # ~ ax.set_ylabel(r'y (mm)')
        
        axs_fig2[1,1].plot(-(21000  -L/2.)/1000, (21000  -L/2.)/1000, marker='x', markersize=3, color='k', linestyle='')
        axs_fig2[1,1].plot(-(6000  -L/2.)/1000, (6000  -L/2.)/1000, marker='o',  markerfacecolor='none', markersize=3,   color='k', linestyle='')
        axs_fig2[1,1].plot((6000  -L/2.)/1000, (6000  -L/2.)/1000, marker='o',  markerfacecolor='none', markersize=3,   color='k', linestyle='')
        ...
        # ~ heatmap = axs_fig2[1,1].pcolormesh(-xx, yy,   wavesum , cmap='gray_r', alpha=0.5) # faster than pcolor
        
        axs_fig2[1,1].set_aspect('equal')

        scalebar = AnchoredSizeBar(axs_fig2[1,1].transData,
                               20, '2 cm', 'upper left', 
                               pad=0.2,
                               color='black',
                               frameon=False,
                               size_vertical=0.3, fontproperties=fontprops)
        axs_fig2[1,1].add_artist(scalebar)
      
        trans = transforms.blended_transform_factory(axs_fig2[1,1].transData, axs_fig2[1,1].get_xticklabels()[0].get_transform())
        axs_fig2[1,1].tick_params(left = False, right = False , labelleft = False ,
                    labelbottom = False, bottom = False)
                    
        axs_fig2[1,1].annotate(str(int(t))+" hr", xy=(39, 52), xycoords='data',xytext=(0, 5), textcoords='offset pixels', color='k', fontsize=16)             
...
out_file='{out}/SIfig_t4_Vcinf.png'.format(out=dir_fig)
