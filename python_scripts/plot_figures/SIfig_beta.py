...
dirstrings=['{inp}/t7_F9_beta150','{inp}/t7_F9_beta250','{inp}/t4_F9_beta150','{inp}/t4_F9_beta250']
t_ints=[40,40,46, 46]

fig, axs = plt.subplots(nrows=2, ncols=2, figsize=(8,8),  layout='constrained')# , layout='constrained',
...
# ~ chemoeffs = [
    # ~ r'adsorption rate $\phi$ $\mathrm{mL}/$(hrs $\cdot$ PFU)',
    # ~ r'adsorption rate $\phi$ $\mathrm{mL}/$(hrs $\cdot$ PFU)'
# ~ ]
    
labeled_axes = []
labeled_axes.append(axs[0,0])
# ~ labeled_axes.append(axs_fig2[1,0])
labeled_axes.append(axs[1,0])
# ~ labeled_axes.append(axs[1,1])
...
for idir, dirstr in enumerate(dirstrings):
    ...
    for it, t in enumerate(times):
        ...
        if int(t) == t_int:
            ...
            if not row_plot:
                axs[row_plot, col_plot].set_title(chemoeffs[col_plot], fontsize=16) # , y=-0.1
...
out_file='{out}/SIfig_betadep.png'.format(out=dir_fig)
