...
dir_fig='../../sims_for_fig2' # directory with input files
dir_io='{inp}/t7_F9'.format(inp=dir_fig)  # directory with input files
...
# compile what timepoints were saved in the PDE integration
savedat=True
...
t_ref=[36] # time point at which snapshot is plotted
...
# load data
    ...
    xx = np.reshape(x, (Nrows, Ncols))/1000. # /1000 converts to millimiters
    print (xx.shape)
    # ~ sys.exit
    ...
    newstate = derivs_plots(R,S,V,E, A,A2)  #compute derivative terms the same way it's done during PDE integration
    ...
    if it==0: # numerically estimate bacteria inoculation position
    ...
    if int(t) in t_ref:
        ...
        grid = gridspec.GridSpec(1, 1, left=0.01, right=0.95, top=0.95, bottom=0.03,
...
dir_io='{inp}/t4_F9'.format(inp=dir_fig)  # directory with input files
...
for it, t in enumerate(times):
    ...
    newstate = derivs_plots(R,S,V,E, A,A2)  #
    ...
    if int(t) in t_ref:
        ...
        fig = plt.figure(figsize=(5,4))
        grid = gridspec.GridSpec(1, 1, left=0.005, right=0.94, top=0.95, bottom=0.03,
             wspace=0.4, hspace=0.35)
        labeled_axes = []
        ax = plt.Subplot(fig, grid[0, 0])
        fig.add_subplot(ax)
