...
dir_fig='../../runs_F8_t7_new_ingredients' # directory with input files
...
etas=[1, 5, 8, 15]
phis=['2e-9', '2e-8', '6e-8', '1e-7']
# ~ titstrings=[r'$\eta=5$',r'$\eta=5$',r'$\eta=10$',r'$\eta=10$']
# ~ t_ints=[54,22,54,54]

# ~ fig2, axs_fig2 = plt.subplots(nrows=2, ncols=2, figsize=(8,8.5),layout='constrained')
# ~ labeled_axes = []
...
grid = gridspec.GridSpec(4, 4, left=0.06, right=0.99, top=0.99, bottom=0.06,
...
for i_eta, eta in enumerate(etas[::-1]):
    for i_phi, phi in enumerate(phis):

    
        dirstr='{inp}/t7_F9_mf_1_gr_fact_1_phi_{phi}_eta_{eta}'
        
        dir_io=dirstr.format(inp=dir_fig,eta=eta,phi=phi)  # directory with input files
        ...
        t_int=min([int(np.amax(times)),40])
        ...
        for it, t in enumerate(times):
            ...
            xnew = B_tot
            intensities = f(np.where(xnew<np.amax(pxtoCFUs[:,1]), xnew, np.amax(pxtoCFUs[:,1]))) # bounds necessary for high yield simulation exceeding maximum measured CFU control, to avoid extrapolation artifacts
            ...
            if int(t) == t_int:
                ...
                ax = plt.Subplot(fig, grid[row,col])
                fig.add_subplot(ax)
                
                
                    
                # ~ heatmap = ax.pcolormesh(xx, yy, intensities, cmap='gray',vmin=maxbacts_convert, vmax=no_bactcontrol) # faster than pcolor
                heatmap_int = ax.pcolormesh(xx, yy, intensities, cmap='gray',vmin=0, vmax=255, shading='auto', linewidth=0,rasterized=True) # faster than pcolor
                
                if row==3: 
                    ax.set_xlabel('{phi}'.format(phi=phi), fontsize=24)
                if col==0: 
                    ax.set_ylabel('{eta}'.format(eta=eta), fontsize=24)
                
                ax.plot((21000  -L/2.)/1000, (21000  -L/2.)/1000, marker='x', markersize=6, color='k', linestyle='')
                ax.plot((6000  -L/2.)/1000, (6000  -L/2.)/1000, marker='o',  markerfacecolor='none', markersize=6,   color='k', linestyle='')
                
                # ~ cbar1 = fig2.colorbar(heatmap_int,ax=ax)
                
                # ~ cbar1.ax.get_yaxis().labelpad = 15
                # ~ cbar1.ax.set_ylabel('pixel intensity', rotation=270,  fontsize = 16)
                ...
                # ~ heatmap = ax.pcolormesh(xx, yy,   wavesum ,norm=colors.LogNorm(vmin=100, vmax=200), cmap='gray_r', alpha=0.5) # faster than pcolor, , alpha=0.5
                heatmap = ax.pcolormesh(xx, yy,   wavesum , cmap='gray_r', alpha=0.5) # faster than pcolor, , alpha=0.5
                ...
                if row==2 and col==1: 
                    ax.annotate("T7 parameters", xy=(-90, 85), xycoords='data',xytext=(0, 5), textcoords='offset pixels', color='k', fontsize=20)
    
        
                # ~ ax.annotate(str(int(t))+" hr", xy=(65, 85), xycoords='data',xytext=(0, 5), textcoords='offset pixels', color='k', fontsize=16)    #
            #
                ax.tick_params(left = False, right = False , labelleft = False ,
                            labelbottom = False, bottom = False)
...
fig.text(0.37,0.003,r'adsorption rate $\phi$ $\mathrm{mL}/$(hrs $\cdot$ PFU)', rotation='horizontal', fontsize=24)
# ~ fig2.text(0.7,0.98,r'high yield', rotation='horizontal', fontsize=20)
fig.text(0.003,0.43,r'lysis rate $\eta$ (1/hrs)', rotation='vertical', fontsize=24)
# ~ fig2.text(0.005,0.15,r'rescaled $\chi$', rotation='vertical', fontsize=20)
...
out_file='{out}/SIfig_phage_params_T7.png'.format(out=dir_fig)
