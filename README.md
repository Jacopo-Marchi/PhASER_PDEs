...
In [the bash scripts folder](./bash_scripts) there are bash handlers used to sweep through the parameters space and generate movies from pre-processed png frames. The [bash wrapper](./bash_scripts/model_batch_runs.sh) sets some of the model parameters, allowing to cycle over them. Once accessing the script folder in a shell terminal, the script can be simply run as ./model_batch_runs.sh. The bash wrapper calls the python [simulation code](./python_scripts/run_model.py) running the PDE integration. The parameters needed to generate all manuscript figures are thoroughly listed in the manuscript's Supplementary Text. Those not specified within the [bash wrapper](./bash_scripts/model_batch_runs.sh) are reported in the [simulation code](./python_scripts/run_model.py) as commented python dictionaries. Next to each dictionary, a comment indicates which Figure panel/condition it corresponds to, so that the desired set of parameters can be uncommented. Throughout the code further comments indicate what other portions need to be uncommented to produce specific conditions, always reporting the corresponding item in the manuscript. The code uploaded in this repository generates the data plotted in Fig.2A. The default parameters can be changed to produce other figures as indicated in the code comments. Note that the code can take 5-20 hours to run depending on the chosen parameters, and the output files typically weight 2-10 GB. 
