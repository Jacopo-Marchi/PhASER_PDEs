...
In [the bash scripts folder](./scripts_generate_figures) there are the bash handlers used to sweep through the parameters space and generate movies from pre-processed frames. The [bash wrapper](./scripts_generate_figures) sets two of the model parameters, and calls the python [simulation code](./scripts_generate_figures) running the PDE integration. The parameters needed to generate all manuscript figures are thoroughly listed in the manuscript's Supplementary Text, and are also reported in the simulation code as commented python dictionaries.
[The plots folder](./plots) contains the python scripts to produce the figures once synthetic data are generated. They reqire specific relative folder placements to find the correct data. Note that the code can take 5-20 hours to run depending on the chosen parameters, and the output files tipically weight 2-10 GB.
